...
Local state evolution is governed by finite linear operators mapped from the Poincaré group ($SO(3,1)$) and internal gauge symmetries. For any arbitrary voxel state vector $\Psi(x^\mu) \in \mathbb{C}^7$, the discrete update rule progressing from time step $t$ to $t + \Delta t$ is:
\begin{equation}
